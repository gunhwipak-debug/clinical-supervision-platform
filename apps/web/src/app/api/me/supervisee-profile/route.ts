...
import { nullable, superviseeProfileSchema } from "@/lib/profiles/validation";
...
export async function GET() {
  ...
  const profile = await withUserContext(
    ...
    (tx) => profiles.getSuperviseeProfileByUserId(tx, current.session.userId)
  );
...
export async function PUT(request: NextRequest) {
  ...
  const parsed = superviseeProfileSchema.safeParse(await parseJson(request));
  if (!parsed.success) {
    return envelope(
      null,
  ...
  const profile = await withUserContext(
    ...
    { userId: current.session.userId, role: current.session.role },
    (tx) =>
      profiles.upsertSuperviseeProfile(tx, current.session.userId, {
