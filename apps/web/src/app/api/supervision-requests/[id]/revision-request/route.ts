...
export async function POST(
  ...
  const result = await withUserContext(
    ...
    async (tx) => {
      ...
      if (!["in_review", "feedback_submitted"].includes(target.status)) {
        return { kind: "invalid_state" as const };
      }
      assertTransition(target.status, "additional_info_requested", "supervisor");
      ...
      if (!cycle) return { kind: "invalid_state" as const };
      ...
      return updated
        ? { kind: "ok" as const, cycle, request: updated }
  ...
  return envelope({ reviewCycle: result.cycle, request: result.request }, null, 200);
}
