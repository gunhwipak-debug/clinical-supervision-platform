...
export function PayoutComputeForm() {
  ...
  async function submit() {
    ...
    try {
      ...
      if (!response.ok) {
        setMessage(body?.error?.message ?? body?.error?.code ?? "정산 계산 실패");
        return;
      }

      const count = body?.data?.payouts?.length ?? 0;
      setMessage(`정산 계산 완료: ${count.toLocaleString("ko-KR")}건`);
      router.refresh();
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="mt-5 grid gap-3 rounded-xl bg-brand-50 p-4">
      ...
      <label className="grid gap-2 text-sm font-semibold text-ink-700">
        <span>왜 이렇게 계산하는지 기록</span>
        <textarea
          className="min-h-20 rounded-xl border border-line bg-surface-elevated px-3 py-2"
          minLength={30}
          onChange={(event) => setReason(event.target.value)}
          placeholder="예: 이번 정산 기간의 결제 완료·환불 완료 내역을 확인하고 정산 계산을 실행합니다."
          value={reason}
        />
        <span className="text-xs font-semibold text-ink-500">
          정산 계산 근거를 30자 이상 직접 입력해야 실행할 수 있습니다.
        </span>
      </label>
